/****************************************************************************************************************************
  ...
  org. code: https://github.com/khoih-prog/Portenta_H7_AsyncWebServer/blob/main/examples/WiFi/Async_PostServer/Async_PostServer.ino 
  machine contol github: https://github.com/arduino-libraries/Arduino_MachineControl/tree/master/examples 
...
int CountDownTimePara = 10000;        // get time in ms
int ExperimentTimePara = 10000;       // get time in ms
int EndingSequensePara = 10000;       // get time in ms
...
float reference = 3.3;  
float Vvalue = 6.5;   // voltage value    // max out 10.4V
...
const String postForms = "<html>\
  ...
  <body>\
    ...
    <form method=\"post\" enctype=\"application/x-www-form-urlencoded\" action=\"/CountDownTime/\">\
      <input type=\"text\" name=\"CountDown_Time\" value=\"10000\">\
...
</html>"; // html code for website

void handleRoot(AsyncWebServerRequest *request) {   // sends html code to website
	request->send(200, "text/html", postForms);
}

void handlePlain(AsyncWebServerRequest *request) {  // if method not allowed
...
void handleForm(AsyncWebServerRequest *request) {   // gets input from website and stores them in thier own variable 
	if (request->method() != HTTP_POST)
	{
		request->send(405, "text/plain", "Method Not Allowed");
	}
...
void handleNotFound(AsyncWebServerRequest *request) {   // if argument is invalid
...
void printWifiStatus() {  // prints wifi status ssid and local ip address
...
void heartBeatPrint() {   // prints in Serial a dot that resembels a heartbeat
	static int num = 1;
...
void paraPrint() {    // prints the saved parameters for the sequense
  Serial.print("\nCountDownTime: ");
...
void check_status() {   // checking staus and runs heartBeatPrint() after a few seconds
...
void machineRun() {   // the sequense code for all the operations
  ...
  Serial.println("Starting machine sequense");
  ...
  while(true) {
   ...
   //=============Countdown Sequense==============================
    ...
    if ((currentMillis - constStartMillis) <= 10) {    // Starts led1
...
void setup() {  // setups for ports and the webserver 
	Serial.begin(115200);
...
void loop() {   // main code that loops and checks if the button has turned on 
  float raw_voltage_ch2 = analog_in.read(2);  // reads analog for button state
  float voltage_ch2 = (raw_voltage_ch2 * reference) / 65535 / res_divider;  // gets the clean voltage value

	check_status();       // some status check 

  if (voltage_ch2 > 2.2) {    // change the num value to about 5.5 when running seriusly if running with built in analog out ////////////////////////////////////////
    Serial.println(voltage_ch2);  // for checking what the voltage was when running the code
    machineRun();           // runs machine code for the sequense
  }
}